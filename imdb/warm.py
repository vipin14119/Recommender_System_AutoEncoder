...
gen_df_dup = gen_df_dup.loc[gen_df_dup[0].isin(rated_movies)]
...
for i in range(0,1):
	...
	with tf.Session() as sess:
		...
		if MAE[i, 0] < BEST_MAE:
			BEST_MAE = MAE[i, 0]
			save_path = saver.save(sess, "./logs/model_lr"+str(learning_rate)+".ckpt")

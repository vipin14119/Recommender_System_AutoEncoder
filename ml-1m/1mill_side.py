...
g_map = {"Action":0,"Adventure":1,"Animation":2,"Children's":3,"Comedy":4,"Crime":5,"Documentary":6,
       "Drama":7,"Fantasy":8,"Film-Noir":9,"Horror":10,"Musical":11,"Mystery":12,"Romance":13,"Sci-Fi":14,"Thriller":15,"War":16
       ,"Western":17}
...
for line in f.readlines():
   ...
   # print(g)
   for genre in g:
       ItemEncoding[movieId-1, g_map[genre]] = 1
# print(ItemEncoding)
...
# first 3 encoders
V1 = tf.Variable(tf.random_uniform([inputSize + 18,hiddenLayer1],-1.0 / math.sqrt(inputSize + 18),1.0 / math.sqrt(inputSize + 18)),trainable=True)
V2 = tf.Variable(tf.random_uniform([hiddenLayer1 + 18 ,hiddenLayer2],-1.0 / math.sqrt(hiddenLayer1+ 18 ),1.0 / math.sqrt(hiddenLayer1+ 18 )),trainable=True)
V3 = tf.Variable(tf.random_uniform([hiddenLayer2 + 18,hiddenLayer3],-1.0 / math.sqrt(hiddenLayer2 + 18),1.0 / math.sqrt(hiddenLayer2 + 18)),trainable=True)
...
S1 = tf.Variable(tf.random_uniform([hiddenLayer1 + 18,inputSize],-1.0 / math.sqrt(hiddenLayer1 + 18),1.0 / math.sqrt(hiddenLayer1 + 18)),trainable=True)
S2 = tf.Variable(tf.random_uniform([hiddenLayer2 + 18,inputSize],-1.0 / math.sqrt(hiddenLayer2 + 18),1.0 / math.sqrt(hiddenLayer2 + 18)),trainable=True)
S3 = tf.Variable(tf.random_uniform([hiddenLayer3 + 18,inputSize],-1.0 / math.sqrt(hiddenLayer3 + 18),1.0 / math.sqrt(hiddenLayer3 + 18)),trainable=True)
S1I = tf.Variable(tf.random_uniform([hiddenLayer1 + 18,18],-1.0 / math.sqrt(hiddenLayer1 + 18),1.0 / math.sqrt(hiddenLayer1 + 18)),trainable=True)
S2I = tf.Variable(tf.random_uniform([hiddenLayer2 + 18,18],-1.0 / math.sqrt(hiddenLayer2 + 18),1.0 / math.sqrt(hiddenLayer2 + 18)),trainable=True)
S3I = tf.Variable(tf.random_uniform([hiddenLayer3 + 18,18],-1.0 / math.sqrt(hiddenLayer3 + 18),1.0 / math.sqrt(hiddenLayer3 + 18)),trainable=True)
...
pi1I = tf.Variable(tf.zeros([18]),trainable=True)
pi2I = tf.Variable(tf.zeros([18]),trainable=True)
pi3I = tf.Variable(tf.zeros([18]),trainable=True)
...
for i in range(0,1):
	...
	for line in traininSet.readlines():
		...
		# print(cc)
		if(tindex < 900209 and cc == trainIndex[tindex]):
	...
	with tf.Session() as sess:
		...
		for k in range(0,1000):
			...
			for q in range(0,1):
				...
				opt,lossP,summary,V_1,V_2,V_3,mu_1,mu_2,mu_3,W_3,W_2,W_1,b_3,b_2,b_1,S_1,S_2,S_3,pi_3,pi_2,pi_1,y_pool_I,y_pool= sess.run([optimizer,loss,merged_summary_op,V1,V2,V3,mu1,mu2,mu3,W3,W2,W1,b3,b2,b1,S1,S2,S3,pi3,pi2,pi1,YpoolI,Ypool],feed_dict={X:ItemMatrix[0:3952, :],mapping: ItemCompletion[0:3952, :],ItemSide:ItemEncoding[0:3952, :]})
		...
		for line in testSet.readlines():
			x = [int(t) for t in line.split('::')]
			# print(cc)
			if(tindex < (1000209-900209) and cc == testIndex[tindex]):
				# UserMatrix[x[0]-1,x[1]-1] = x[2]
				ItemMatrix[x[1]-1,x[0]-1] = x[2]
				# UserCompletion[x[0]-1,x[1]-1] = 1
				ItemCompletion[x[1]-1,x[0]-1] = 1
		...
		if MAE[i, 0] < BEST_MAE:
			BEST_MAE = MAE[i, 0]
			save_path = saver.save(sess, "./logs/model_with_0_lr_"+str(learning_rate)+".ckpt")

...
YS1 = tf.identity(tf.add(tf.matmul(E1,S1),pi1))
YS2 = tf.identity(tf.add(tf.matmul(E2,S2),pi2))
YS3 = tf.identity(tf.add(tf.matmul(E3,S3),pi3))
Ypool = (YS1 + YS2 + YS3)/3
...
difference1NM = tf.multiply(X - YS1,mapping)
difference2NM = tf.multiply(X - YS2,mapping)
difference3NM = tf.multiply(X - YS3,mapping)
differencePool = tf.multiply(X - Ypool,mapping)
	...
	with tf.Session() as sess:
		...
		for k in range(0,450):
			...
			for q in range(0,1):
				...
				opt,lossP,summary,V_1,V_2,V_3,mu_1,mu_2,mu_3,W_3,W_2,W_1,b_3,b_2,b_1,S_1,S_2,S_3,pi_3,pi_2,pi_1,Y= sess.run([optimizer,loss,merged_summary_op,V1,V2,V3,mu1,mu2,mu3,W3,W2,W1,b3,b2,b1,S1,S2,S3,pi3,pi2,pi1,Ypool],feed_dict={X:UserMatrix,mapping:UserCompletion})
				summary_writer.add_summary(summary,  k*1 + q)
			print(lossP)
			#print(Y)
			#print(UserMatrix)
			#print(np.sum(np.absolute(PredictionMatrix - np.multiply(Y,PredictionCompletion)))/np.sum(PredictionCompletion))
		...
		save_path = saver.save(sess, "./logs/model_jester2.ckpt")
